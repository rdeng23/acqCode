...
def reAssembleFile(fromFilePath, channelCount, acqTime, fireNum, output_path, das_addbase):
    ...
    localPath = os.path.join(out_dir, "DATA", str(fireNum) + "BLA.DAT")
    dasPath = os.path.join(das_dir, "DATA", str(fireNum) + "BLA.DAT")
    # 写DAT文件
    dataAssemble.tofile(localPath)
    dataAssemble.tofile(dasPath)
    # 写INF文件
    infData = genINF(fireNum, 160, acqTime)
    infPath = os.path.join(out_dir, "INF", str(fireNum) + "BLA.INF")
    with open(infPath, "wb") as fp:
        fp.write(infData)
    infDasPath = os.path.join(das_dir, "INF", str(fireNum) + "BLA.INF")
